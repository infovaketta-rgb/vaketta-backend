...
async function blockToken(jti, exp) {
    ...
    if (ttl > 0) {
        await redis_1.redis.setex(`${JTI_PREFIX}${jti}`, ttl, "1");
    }
}
/**
...
async function invalidateUserTokens(userId) {
    ...
    await redis_1.redis.setex(`${USER_PREFIX}${userId}`, MAX_TOKEN_TTL_SEC, String(now));
}
/**
 ...
 */
async function isTokenBlocked(jti, userId, iat) {
    const [jtiBlocked, userInvalidatedAt] = await redis_1.redis.mget(`${JTI_PREFIX}${jti}`, `${USER_PREFIX}${userId}`);
    if (jtiBlocked)
        return true;
    if (userInvalidatedAt && iat < Number(userInvalidatedAt))
        return true;
    return false;
}
